...
test('email includes both totals but no therapist names or slot preview', () => {
  const report = {date:'2026-09-23',completedDate:'2026-09-22',total:2,completedTotal:1,completedHeld:1,
  ...
  for(const body of [mail.text,mail.html]) {
    ...
    expect(body).toContain('2026-09-22: 1 slots across 1 therapists — 1 marked completed');
...
test('empty days still have explicit zero totals',()=>{
  const text = buildEmail({date:'2026-09-23',completedDate:'2026-09-22',total:0,completedTotal:0,completedHeld:0,groups:[],completedGroups:[]}).text;
  expect(text).toContain('0 slots across 0 therapists');
  expect(text).toContain('0 slots across 0 therapists — 0 marked completed');
});
