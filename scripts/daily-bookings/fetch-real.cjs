...
async function main() {
  ...
  const [sessions, therapists, completed] = await Promise.all([
    ...
    all(() => db.from('sessions').select('id,psychologist_id,scheduled_date,scheduled_time,session_type,status,wix_payload,wix_booking_id,completion_date').eq('status', 'completed').or(`completion_date.eq.${completedDate},and(completion_date.is.null,scheduled_date.eq.${completedDate})`).order('id')),
  ]);
  ...
  for (const [records, targetGroups] of [[sessions, groups], [completed, completedGroups]]) {
    for (const s of records) {
      const g = targetGroups.find(g => g.id === s.psychologist_id);
      if (!g) throw new Error('Booking has unresolved therapist');
      ...
      g.slots.push({ time: s.scheduled_time, type: s.session_type, minutes, status: s.status, scheduledDate: s.scheduled_date || (records === sessions ? date : null) });
    }
  ...
  const report = { date, completedDate, fetchedAt: new Date().toISOString(), total: sessions.length, completedTotal: completed.length, groups, completedGroups };
  fs.mkdirSync(outputDir, { recursive: true });
  fs.writeFileSync(path.join(outputDir, 'real-report.json'), JSON.stringify(report, null, 2));
  console.log(JSON.stringify({ date, completedDate, bookings: sessions.length, completed: completed.length, therapists: groups.length }));
}
